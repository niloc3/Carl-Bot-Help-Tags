...
function bindEvents() {
  ...
  Docsify.dom.on($input, "input", (e) => {
    ...
    timeId = setTimeout(
      () =>
        SearchInputOnPage(
          e.target.value.trim()
        ) /* ListSearcMatches(e.target.value.trim())*/,
      400
    );
  });
  Docsify.dom.on($inputWrap, "click", (e) => {
    ...
    if (e.target.tagName !== "INPUT") {
      ...
      // ClearLabelStyle()
    } else if (!$input.value) ClearLabelStyle();
  });
}
...
function ClearLabelStyle() {
  ...
  for (let i = 0; i < labels.length; i++) labels[i].style = "";
}
    ...
    if (match) {
      if (setLabel) {
        ...
        item.innerHTML = item.innerHTML.replace(
          re,
          `<label style='background-color:${configs.RESULT_COLOR}'>${searchKey}</label>`
        );
      }
      itemIndexInPage += match.length;
      if (itemIndexInPage >= itemIndex) return item;
    }
  } else console.log("yokkk");
